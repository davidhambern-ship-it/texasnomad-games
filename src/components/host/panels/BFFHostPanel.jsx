  ...
  if (isPlaying) {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        {/* ── 1. GAMEBOARD ── */}
        <div className="p-4 border border-[#BC13FE]/20 rounded-xl bg-black/60 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {isHostSittingIn && (
                <div className="px-2 py-1 rounded border border-[#BC13FE]/50 bg-[#BC13FE]/10 text-[8px] text-[#BC13FE] tracking-widest uppercase" style={sty}>HOST IN GAME</div>
              )}
            </div>
          ...
          <div className="space-y-2 pt-1 border-t border-white/10">
            <div className="flex items-center justify-between">
              ...
              <div className="text-[8px] tracking-widest text-white/20 uppercase" style={sty}>Round {gs.round || 1} — {surveys.length} surveys</div>
            </div>
            {gs.current_question ? (
          ...
          <div className="space-y-3 pt-1 border-t border-white/10">
            <div className="flex items-center justify-between">
              <h3 className="font-heading text-xs tracking-[0.2em] text-white/40 uppercase">Answer Board</h3>
          ...
          {/* Player Submission */}
          {lastSub && (
            <div className="p-4 border-2 rounded-xl flex items-center gap-3 mt-2"
          ...
          <div className="space-y-3 pt-1 border-t border-white/10">
            <div className="flex items-center justify-between">
              <h3 className="font-heading text-xs tracking-[0.2em] text-[#4ade80]/70 uppercase">
                Who Answers Now?{gs.answering_player_id && <span className="ml-2 text-[#4ade80]">✓</span>}
              </h3>
              <div className="flex gap-2">
                {gs.answering_player_id && (
                  <Btn onClick={() => advanceToNextPlayer(gs.answering_player_id, gs.active_turn || 1)} color="#FFD700" size="sm">Next Player →</Btn>
                )}
            ...
            {gs.answering_player_id && (
              <div className="flex gap-3">
                <div className="flex-1 px-3 py-2 rounded-lg border border-[#4ade80]/40 bg-[#4ade80]/5 text-center">
                  ...
                  <div className="font-heading text-sm text-[#4ade80]">SEAT {getPlayer(gs.answering_player_id)?.seatNumber || '?'}</div>
                </div>
                {nextAnsweringPlayer && (
          ...
          <div className="pt-1 border-t space-y-3"
            ...
            <div className="flex items-center justify-between">
              <h3 className="font-heading text-xs tracking-[0.2em] text-white/40 uppercase">BYE Strikes {is2PMode ? '— 2P Mode' : '— Full Family'}</h3>
              {byeCount >= 3 && (
            ...
            {is2PMode ? (
              ...
              <div className="flex justify-center py-2"><HostBYEIndicator byeCount={byeCount} /></div>
            )}
            {stealMode && stealPlayer && (
              <div className="p-3 border border-[#FF5F1F] rounded-xl bg-[#FF5F1F]/10 text-center">
                <div className="text-[8px] text-[#FF5F1F] tracking-widest uppercase" style={sty}>🎯 Steal in Progress — Seat {stealPlayer.seatNumber}</div>
                <div className="text-[7px] text-white/40 uppercase mt-1" style={sty}>{stealPlayer.familyTeam === 1 ? (gs.family1 || 'Family 1') : (gs.family2 || 'Family 2')} is attempting steal</div>
              </div>
            )}
            {gs.steal_result && (
        ...
        <div className="p-4 border border-[#BC13FE]/30 rounded-xl bg-black/60 space-y-3">
          ...
          {!isHostSittingIn ? (
            <div className="space-y-2">
              <div className="text-[8px] tracking-widest text-white/30 uppercase" style={sty}>Join as a player while keeping host controls</div>
              <div className="grid grid-cols-2 gap-2">
                <Btn onClick={() => hostSitIn(1)} color="#BC13FE" size="sm">Join {gs.family1 || 'Family 1'}</Btn>
                <Btn onClick={() => hostSitIn(2)} color="#FF5F1F" size="sm">Join {gs.family2 || 'Family 2'}</Btn>
              </div>
              <Btn onClick={() => hostSitIn(hostSitInFamily)} color="#FFD700" size="sm" className="w-full">Auto-assign (smaller team)</Btn>
            </div>
          ) : (
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[8px] tracking-widest text-[#BC13FE] uppercase mb-1" style={sty}>You are playing as Seat {hostPlayerRecord?.seatNumber}</div>
                <div className="text-[7px] text-white/30 uppercase" style={sty}>{hostPlayerRecord?.familyTeam === 1 ? (gs.family1 || 'Family 1') : (gs.family2 || 'Family 2')} — Host Player</div>
              </div>
        ...
        {/* ── 3. COMBINED FACEOFF CONTROL ── */}
        <div className="p-4 border rounded-xl bg-black/60 space-y-3"
          style={{ borderColor: faceoffMode ? '#FF5F1F60' : '#FF5F1F20' }}>
          <div className="flex items-center justify-between">
            <h3 className="font-heading text-xs tracking-[0.2em] text-[#FF5F1F]/70 uppercase">⚡ Faceoff Control</h3>
            <div className="flex gap-2">
              {faceoffMode && <Btn onClick={startFaceoff} color="#FF5F1F" size="sm">↺ Rotate</Btn>}
              {faceoffMode && <Btn onClick={endFaceoff} color="#ffffff" size="sm">End Faceoff</Btn>}
              {!faceoffMode && !is2PMode && <Btn onClick={startFaceoff} color="#FF5F1F" size="sm">⚡ Start Faceoff</Btn>}
            </div>
          </div>

          {/* Current faceoff matchup */}
          {fo1 && fo2 ? (
            <div className="px-4 py-3 rounded-xl border border-[#FF5F1F]/40 bg-[#FF5F1F]/5 text-center">
              ...
              <div className="flex justify-center gap-8 mt-2">
                <div className="text-center">
                  <div className="text-[7px] text-white/30 uppercase mb-1" style={sty}>{gs.family1 || 'Family 1'}</div>
                  <div className="font-heading text-lg text-[#BC13FE]">SEAT {fo1.seatNumber}</div>
                </div>
                <div className="font-heading text-xl text-white/30 self-center">VS</div>
                <div className="text-center">
                  <div className="text-[7px] text-white/30 uppercase mb-1" style={sty}>{gs.family2 || 'Family 2'}</div>
                  <div className="font-heading text-lg text-[#FF5F1F]">SEAT {fo2.seatNumber}</div>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center text-[8px] text-white/20 tracking-widest uppercase py-2" style={sty}>No faceoff players selected</div>
          )}

          {/* Faceoff player pickers */}
          <div className="grid grid-cols-2 gap-4">
