...
export default function HandSetup({ suitOrder, onSuitOrderChange, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-gradient-to-br from-[#0a0a1a] to-[#050510] border-2 border-[#BC13FE]/50 rounded-2xl p-5 max-w-sm w-full shadow-2xl"
          ...
          {SUIT_PRESETS.map((preset, i) => {
            ...
            return (
              <button key={i} onClick={() => applyPreset(preset)}
                className={`px-3 py-2.5 rounded-xl border-2 text-left transition-all ${isActive ? 'border-[#BC13FE] bg-[#BC13FE]/20' : 'border-white/15 bg-white/5 hover:border-white/30'}`}>
                <div className="text-[6px] tracking-widest uppercase mb-1" style={{ ...PS2, color: isActive ? '#BC13FE' : 'rgba(255,255,255,0.5)' }}>
                  {preset.label}
                </div>
                {preset.suits && (
                  <div className="flex gap-1">
                    {preset.suits.map(s => (
                      <span key={s} style={{ color: SUIT_COLORS[s], fontSize: 14 }}>{s}</span>
                    ))}
                  </div>
                )}
                {preset.suits === null && (
                  <div className="text-white/30 text-[8px]">Drag to reorder</div>
                )}
              </button>
            );
          })}
        ...
        {isCustom && (
          <div className="border-t border-white/10 pt-4">
            <div className="text-[6px] text-white/40 uppercase mb-2 tracking-widest" style={PS2}>Drag to reorder</div>
            <div className="flex flex-col gap-2">
              {customOrder.map((suit, idx) => (
                <div key={suit}
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 cursor-grab active:cursor-grabbing transition-all select-none ${
                    dragOver === idx ? 'border-[#BC13FE] bg-[#BC13FE]/20 scale-105' : 'border-white/15 bg-white/5'
                  }`}>
                  <span className="text-white/30 text-xs">⠿</span>
                  <span style={{ color: SUIT_COLORS[suit], fontSize: 20 }}>{suit}</span>
                  <span className="text-white/70 text-sm font-body">{SUIT_NAMES[suit]}</span>
                  <span className="ml-auto text-white/20 text-[9px]" style={PS2}>#{idx + 1}</span>
                </div>
              ))}
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              {customOrder.map((suit, idx) => (
                <div key={idx} className="flex flex-col gap-1">
                  <div className="text-[6px] text-white/30 uppercase" style={PS2}>#{idx + 1}</div>
                  <select
                    value={suit}
        ...
        <div className="mt-4 pt-3 border-t border-white/10">
          <div className="text-[6px] text-white/30 uppercase mb-2 tracking-widest" style={PS2}>Preview Order</div>
          <div className="flex gap-3 justify-center">
            {(isCustom ? customOrder : (SUIT_PRESETS.find(p => JSON.stringify(p.suits) === JSON.stringify(customOrder))?.suits || customOrder)).map((s, i) => (
              <div key={s} className="flex flex-col items-center gap-1">
                <span style={{ color: SUIT_COLORS[s], fontSize: 22 }}>{s}</span>
                <span className="text-[5px] text-white/30" style={PS2}>{i + 1}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <button onClick={onClose}
          className="mt-4 w-full py-2.5 rounded-xl border-2 border-[#BC13FE] text-[#BC13FE] text-[7px] tracking-widest uppercase hover:bg-[#BC13FE]/20 transition-all"
          style={PS2}>
          Done
        </button>
      </div>
