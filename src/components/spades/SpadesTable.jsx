          ...
          <div className="absolute z-10 pointer-events-none"
            ...
            {Array.from({ length: Math.min(localCount, 7) }).map((_, i, arr) => {
              ...
              return (
                <div key={i} style={{
                  width: 32, height: 46,
                  marginLeft: i > 0 && (position === 'top' || position === 'bottom') ? '-16px' : '0',
                  marginTop: i > 0 && (position === 'left' || position === 'right') ? '-28px' : '0',
...
function getOpponentHandStyle(position) {
  switch (position) {
    ...
    case 'left':   return { left: 8,    top: '50%',  transform: 'translateY(-50%)', display: 'flex', flexDirection: 'column' };
