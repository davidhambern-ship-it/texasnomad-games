          ...
          <>
            ...
            {player?.hand?.length > 0 && !isMe && (
              <div className="mt-1 flex justify-center gap-0.5">
                {[...player.hand].slice(0, Math.min(5, player.hand.length)).map((card, i) => (
                  <div key={i} className="w-4 h-6 rounded border border-white/20 overflow-hidden shadow-sm"
                    style={{ transform: `rotate(${(i - 2) * 3}deg)`, marginLeft: i > 0 ? '-6px' : '0' }}>
                    <img src={getCardBack()} alt="Card" className="w-full h-full object-cover" />
                  </div>
                ))}
                {player.hand.length > 5 && (
                  <div className="w-4 h-6 rounded border border-white/20 bg-black/60 flex items-center justify-center text-[5px] text-white/60" style={{ marginLeft: '-6px' }}>
                    +{player.hand.length - 5}
                  </div>
                )}
              </div>
