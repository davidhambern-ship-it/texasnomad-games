...
function wsUrl(roomCode, role, identity) {
  ...
  const configuredBase = String(import.meta.env.VITE_BFF_VOICE_BASE || '').trim();
  const onRailway = window.location.hostname.endsWith('.up.railway.app');
  ...
  return `${socketBase.replace(/\/$/, '')}/bff-voice?${params.toString()}`;
}
